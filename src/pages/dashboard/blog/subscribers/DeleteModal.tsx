...
const DeleteModal = ({ isShow, closeModal, modalTitle, flattened, stretch, slim, data, type }: Partial<IDeleteSubModal>) => {
    ...
    const deleteSubscriber = async (e: any) => {

        if(e){ e.preventDefault(); }

        setLoading(true)

        await Axios.delete(`${process.env.NEXT_PUBLIC_BLOG_URL}/subscribers/${data._id}`, storage.getConfigWithBearer())
        .then((resp) => {

            if(resp.data.error === false && resp.data.status === 200){
                navigate.push('/dashboard/blog/subscribers')
                reloadData()
                closeX(e);
            }
    ...
    const enableDisable = async (e: any, t: string) => {
        ...
        if(t === 'enable'){
            url = `${process.env.NEXT_PUBLIC_BLOG_URL}/subscribers/${t}/${data._id}`
        }

        if(t === 'disable'){
            url = `${process.env.NEXT_PUBLIC_BLOG_URL}/subscribers/${t}`
        }

        setLoading(true)

        await Axios.put(url, { email: data.email } , storage.getConfigWithBearer())
        .then((resp) => {
    ...
    const reloadData = () => {
        ...
        if(type === 'list'){
            ...
            if(userContext.getUserType() === 'superadmin' || userContext.getUserType() === 'admin'){
                blogContext.getSubscribers(30, 1);
            }

        }

        if(type === 'single'){
            blogContext.getSubscriber(data._id);
        }
                                        ...
                                        <div className="mrgb1">
                                            ...
                                            <div className='row mrgb1'>
                                                <div className='col-md-8 mx-auto ui-text-center'>
                                                    <p className="font-frei fs-16 mrgb1 ui-line-height-medium mrgb0" style={{ color: '#C3C2FB' }}>
                                                        Are you sure you want to delete this subscriber? You can just { data && data.isEnabled ? 'disable' : 'enable' }. 
                                                    </p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="ui-text-center">
                                               <div className='mrgb1'>
                                                    ...
                                                    <Link onClick={(e) => deleteSubscriber(e)} href="" className={`btn sm bgd-disable onwhite ${loading ? 'disabled-lt' : ''}`}>
...

                                {
                                    ...
                                    <>
                                        ...
                                        <Message
                                            title="Successful!"
                                            displayTitle={false}
                                            message={data && data.isEnabled ? 'Subscriber disabled successfully' : 'Subscriber enabled successfully'}
                                            action={closeX}
                                            status="success"
                                            lottieSize={200}
                                            loop={false}
