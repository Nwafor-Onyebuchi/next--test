...
const DeleteModal = ({ isShow, closeModal, modalTitle, flattened, stretch, slim, data, type }: Partial<IDeletePostModal>) => {
    ...
    const deletePost = async (e: any) => {
        ...
        await Axios.delete(`${process.env.NEXT_PUBLIC_BLOG_URL}/posts/${data._id}`, storage.getConfigWithBearer())
        .then((resp) => {
    ...
    const enableDisable = async (e: any, t: string) => {
        ...
        await Axios.put(`${process.env.NEXT_PUBLIC_BLOG_URL}/posts/${t}/${data._id}`, {}, storage.getConfigWithBearer())
        .then((resp) => {
    ...
    const reloadData = () => {
        ...
        if(type === 'list'){
            ...
            if(userContext.getUserType() === 'writer'){
                blogContext.getUserPosts(30, 1, storage.getUserID())
            }

            if(userContext.getUserType() === 'superadmin' || userContext.getUserType() === 'admin'){
                blogContext.getPosts(30, 1);
            }

        }

        if(type === 'single'){
            blogContext.getPost(data._id);
        }
                                        ...
                                        <div className="mrgb1">
                                            ...
                                            <div className='row mrgb1'>
                                                <div className='col-md-8 mx-auto ui-text-center'>
                                                    <p className="font-frei fs-16 mrgb1 ui-line-height-medium mrgb0" style={{ color: '#C3C2FB' }}>
                                                        Are you sure you want to delete this post? You can as well just { data && data.isEnabled ? 'disable' : 'enable' } it. 
                                                    </p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="ui-text-center">
                                               <div className='mrgb1'>
                                                    ...
                                                    <Link onClick={(e) => deletePost(e)} href="" className={`btn sm bgd-disable onwhite ${loading ? 'disabled-lt' : ''}`}>
...

                                {
                                    ...
                                    <>
                                        ...
                                        <Message
                                            title="Successful!"
                                            displayTitle={false}
                                            message={data && data.isEnabled ? 'Post disabled successfully' : 'Post enabled successfully'}
                                            action={closeX}
                                            status="success"
                                            lottieSize={200}
                                            loop={false}
