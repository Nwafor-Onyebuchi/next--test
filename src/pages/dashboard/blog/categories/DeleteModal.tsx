...
const DeleteModal = ({ isShow, closeModal, modalTitle, flattened, stretch, slim, data, type }: Partial<IDeleteSubModal>) => {
    ...
    const deleteCategory = async (e: any) => {
        ...
        await Axios.delete(`${process.env.NEXT_PUBLIC_BLOG_URL}/categories/${data._id}`, storage.getConfigWithBearer())
        .then((resp) => {
    ...
    const enableDisable = async (e: any, t: string) => {
        ...
        await Axios.put(`${process.env.NEXT_PUBLIC_BLOG_URL}/categories/${t}/${data._id}`, { } , storage.getConfigWithBearer())
        .then((resp) => {
    ...
    const reloadData = () => {
        ...
        if (userContext.isAdmin || userContext.isSuper) {
                                        ...
                                        <div className="mrgb1">
                                            ...
                                            <div className='row mrgb1'>
                                                <div className='col-md-8 mx-auto ui-text-center'>
                                                    <p className="font-frei fs-16 mrgb1 ui-line-height-medium mrgb0" style={{ color: '#C3C2FB' }}>
                                                        Are you sure you want to delete this category? You can just { data && data.isEnabled ? 'disable' : 'enable' }. 
                                                    </p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="ui-text-center">
                                               <div className='mrgb1'>
                                                    ...
                                                    <Link onClick={(e) => deleteCategory(e)} href="" className={`btn sm bgd-disable onwhite ${loading ? 'disabled-lt' : ''}`}>
...

                                {
                                    ...
                                    <>
                                        ...
                                        <Message
                                            title="Successful!"
                                            displayTitle={false}
                                            message={data && data.isEnabled ? 'Category disabled successfully' : 'Category enabled successfully'}
                                            action={closeX}
                                            status="success"
                                            lottieSize={200}
                                            loop={false}
