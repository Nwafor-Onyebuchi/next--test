...
const DeleteModal = ({ isShow, closeModal, modalTitle, flattened, stretch, slim, data, type }: Partial<IDeleteSubModal>) => {
    ...
    const deleteTag = async (e: any) => {

        if(e){ e.preventDefault(); }

        setLoading(true)

        await Axios.delete(`${process.env.NEXT_PUBLIC_BLOG_URL}/tags/${data._id}`, storage.getConfigWithBearer())
        .then((resp) => {

            if(resp.data.error === false && resp.data.status === 200){
                navigate.push('/dashboard/blog/tags')
                reloadData()
                closeX(e);
            }
    ...
    const enableDisable = async (e: any, t: string) => {
        ...
        await Axios.put(`${process.env.NEXT_PUBLIC_BLOG_URL}/tags/${t}/${data._id}`, { } , storage.getConfigWithBearer())
        .then((resp) => {
                                        ...
                                        <div className="mrgb1">
                                            ...
                                            <div className='row mrgb1'>
                                                <div className='col-md-8 mx-auto ui-text-center'>
                                                    <p className="font-frei fs-16 mrgb1 ui-line-height-medium mrgb0" style={{ color: '#C3C2FB' }}>
                                                        Are you sure you want to delete this tag? { (userContext.isSuper || userContext.isAdmin) ? ` You can just ${ data && data.isEnabled ? 'disable' : 'enable' } ` : '' }. 
                                                    </p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="ui-text-center">
                                               <div className='mrgb1'>
                                                    ...
                                                    <Link onClick={(e) => deleteTag(e)} href="" className={`btn sm bgd-disable onwhite ${loading ? 'disabled-lt' : ''}`}>
...

                                {
                                    ...
                                    <>
                                        ...
                                        <Message
                                            title="Successful!"
                                            displayTitle={false}
                                            message={data && data.isEnabled ? 'Tag disabled successfully' : 'Tag enabled successfully'}
                                            action={closeX}
                                            status="success"
                                            lottieSize={200}
                                            loop={false}
